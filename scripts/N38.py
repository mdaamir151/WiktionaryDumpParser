...
regex = re.compile(r'{{(en-noun)(\|)?(?P<plural>.*?)}}')
...
def repl(matchObj):
	...
	if noplural:
		return "'''"+head+"'''"
	if uncountable and mult == '':
		return "'''"+head+"''' (''uncountable'')"
	elif uncountable:
	...
	elif countAndUncount:
		return "'''"+head+"''' (''countable and uncountable''; ''plural'' "+headword+"s)"
	elif mult != '':
		return "'''"+head+"''' (''plural'' "+mult+")"
	else:
		return "'''"+head+"''' (''plural'' "+headword+"s)"
